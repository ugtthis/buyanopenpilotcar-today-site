export type SupportContentSection = {
  title: string;
  paragraphs: string[];
...
export type SupportTypeContent = {
  paragraphs: string[];
  expandableContent?: {
    sections: SupportContentSection[];
  };
...
export const SUPPORT_TYPE_CONTENT: Record<string, SupportTypeContent> = {
  ...
  custom: {
    paragraphs: [
      "This vehicle is supported via a custom openpilot fork maintained outside " +
      "of the official upstream repository.",
      "Custom forks may behave differently from upstream openpilot and are not " +
      "subject to comma's standard safety review process."
    ],
    reference: {
      text: "openpilot wiki",
      url: "https://github.com/commaai/openpilot/wiki"
